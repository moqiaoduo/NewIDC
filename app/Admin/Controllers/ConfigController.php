...
class ConfigController extends Controller
{
    ...
    public function index(Content $content)
    {
        ...
        $form->fill(getOptions([
            'tos','tos_url','admin_email',
            'service_username_generation',
            'site_service_username_unique',
            'site_service_domain_unique'
        ]));
        ...
        return $this->pub($content,'base',$form);
    }
    ...
    public function cron(Content $content)
    {
        ...
        $form->number('suspend_after_days','过期n天后暂停');
        $form->switch('terminate_expire','停止过期服务');
        $form->number('terminate_after_days','过期n天后停止');
        return $this->pub($content,'cron',$form);
    }
    ...
    public function template(Content $content)
    {
        ...
        $form->multipleSelect('template_default_home_product','首页显示推荐产品')->options(function ($ids) {
            if ($ids)
                return Product::find($ids)->pluck('name', 'id');
        })->ajax('/admin/api/products')->config('maximumSelectionLength',4)
            ->help('选中的产品将在首页展示，最多4个');
        return $this->pub($content,'template',$form);
    }

    public function third_part(Content $content)
    {
        $nowTab=\request()->route()->fallbackPlaceholder;
        $form=new Form();
        $form->method('POST')->action($this->save_url);
